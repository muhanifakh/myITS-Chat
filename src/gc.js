...
import { ArrowLeft, User, Plus, Send, Paperclip, Image, FileText, Reply, Pin, Trash2 } from 'lucide-react';
import GroupInfoPage from './gcprof.js'; // Import the Group Info component

const GroupChatPage = ({ groupName = "Machine Learning (1)", onBack, onDeleteGroup }) => {
  ...
  const [messages, setMessages] = useState([
    ...
    },
    {
      id: 7,
      sender: "Arthur",
      content: "In the report structure it's claimed that we should implement 3 different Supervised Learning methods while before that it was only 2",
      timestamp: "10:36",
      isOwn: false,
      type: "text"
    },
    {
      id: 8,
      sender: "Arthur",
      content: "Can you explain that difference please",
      timestamp: "10:37",
      isOwn: false,
      type: "text"
    }
  ...
  const handlePin = () => {
    ...
    if (isPinned) {
      setPinnedMessages(prev => prev.filter(pm => pm.id !== selectedMessage.id));
    } else {
  ...
  const handleFileUpload = (fileType) => {
    ...
    const fileNames = {
      image: ['vacation.jpg', 'screenshot.png', 'photo.jpeg'],
      document: ['report.pdf', 'presentation.pptx', 'notes.docx'],
      general: ['archive.zip', 'data.csv', 'config.json']
    };
  ...
  // Show group info page if user clicked on user icon
  if (showGroupInfo) {
    return (
      <GroupInfoPage 
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        <h1 className="text-lg font-semibold text-gray-900 flex-1 text-center">{groupName}</h1>
        <button 
